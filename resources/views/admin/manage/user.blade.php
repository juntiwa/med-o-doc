...
<section id="button" class="flex items-end justify-end w-full">
    ...
    <a href="{{route('manage.create')}}">
        <label
            class="modal-button flex items-end justify-end w-fit px-3 py-2 rounded-md bg-blue-900 hover:bg-blue-800 text-white cursor-pointer">
                    ...
                    <td class="p-3 text-base text-gray-700 whitespace-nowrap">
                        <div class="flex">
                            ...
                            <label class="swap swap-rotate cursor-pointer">
                                ...
                                <input type="checkbox" id="{{$user->org_id}}"
                                    class="switch{{$user->org_id}} sr-only peer" onClick="sap_click(this.id)" />
                                <svg class="swap-on fill-current w-5 h-5 ml-3" xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 24 24">
                                    <path
                                        d="M21.92,11.6C19.9,6.91,16.1,4,12,4S4.1,6.91,2.08,11.6a1,1,0,0,0,0,.8C4.1,17.09,7.9,20,12,20s7.9-2.91,9.92-7.6A1,1,0,0,0,21.92,11.6ZM12,18c-3.17,0-6.17-2.29-7.9-6C5.83,8.29,8.83,6,12,6s6.17,2.29,7.9,6C18.17,15.71,15.17,18,12,18ZM12,8a4,4,0,1,0,4,4A4,4,0,0,0,12,8Zm0,6a2,2,0,1,1,2-2A2,2,0,0,1,12,14Z" />
                                </svg>
                                <svg class="swap-off fill-current w-5 h-5 ml-3" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <td class="p-3 text-base text-gray-700 whitespace-nowrap">
                        ...
                        <label for="my-modal-{{$user->id}}" class="modal-button">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                class="fill-blue-400 hover:fill-blue-600 cursor-pointer">
                                <path
                                    d="M4 21a1 1 0 0 0 .24 0l4-1a1 1 0 0 0 .47-.26L21 7.41a2 2 0 0 0 0-2.82L19.42 3a2 2 0 0 0-2.83 0L4.3 15.29a1.06 1.06 0 0 0-.27.47l-1 4A1 1 0
                        ...
                        <div class="modal">
                            <div class="modal-box relative bg-white w-11/12 max-w-5xl">
                                <label for="my-modal-{{$user->id}}"
                                    class="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
                                <h3 class="text-lg font-bold">แก้ไขข้อมูลผู้ใช้งาน</h3>
                                <p class="py-4">
                                <section id="formEdit">
                                    <form class="space-y-6 w-full" action="{{ route('manage.update',$user->org_id) }}"
                                        method="post">
                                        @csrf
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label for="org_id"
                                                    class="block mb-2 text-base font-medium text-gray-900">รหัสพนักงาน
                                                ...
                                                focus:ring-teal-500 focus:border-teal-500 block w-full p-2.5"
                                                    placeholder="รหัสพนักงาน" value="{{$user->org_id}}" required>
                                            </div>
                                            <div>
                                                <label for="login"
                                                    class="block mb-2 text-base font-medium text-gray-900">ชื่อผู้ใช้งาน</label>
                                                <input type="text" name="login" id="login" class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg
                                                focus:ring-teal-500 focus:border-teal-500 block w-full p-2.5"
                                                    placeholder="ชื่อผู้ใช้งาน" readonly value="{{$user->username}}">
                                            </div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label for="full_name"
                                                    class="block mb-2 text-base font-medium text-gray-900">ชื่อ สกุล
                                                </label>
                                                <input type="text" name="full_name" id="full_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-base rounded-lg
                                                focus:ring-teal-500 focus:border-teal-500 block w-full p-2.5"
                                                    placeholder="ชื่อ นามสกุล" readonly value="{{$user->full_name}}">
                                            </div>
                                            <div>
                                                <label for="office_name"
                                                    class="block mb-2 text-base font-medium text-gray-900">หน่วยงาน</label>
                                             ...
                                             focus:ring-teal-500 focus:border-teal-500 block w-full p-2.5 ">
                                                    ...
                                                    <option value="{{$unit->unitid}}" {{ ($user->office_name ==
                                                        $unit->unitname ? "selected": "") }}> {{$unit->unitname}}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                <label for="is_admin"
                                                    class="block mb-2 text-base font-medium text-gray-900">สิทธิ์ผู้ใช้งาน
                                                    <span class="text-rose-600">*</span></label>
                                            ...
                                            <div>
                                                <label for="status"
                                                    class="block mb-2 text-base font-medium text-gray-900">สถานะการใช้งาน
                                                    <span class="text-rose-600">*</span></label>
                                        ...
                                        <div class="flex items-end justify-end w-fit">
                                            <button type="submit"
                                                class="w-full text-white bg-teal-700 hover:bg-teal-800 focus:ring-4 focus:outline-none
            ...
            <div class="flex justify-between space-x-2 text-base">
                <div class="flex">
                    ...
                    <div class="text-gray-500 flex">
                        ...
                        <label class="swap swap-rotate cursor-pointer">
                            <!-- this hidden checkbox controls the state -->
                            <input type="checkbox" id="{{$user->org_id}}" class="switch{{$user->org_id}} sr-only peer"
                                onClick="sap_click(this.id)" />
                            <svg class="swap-on fill-current w-5 h-5 ml-3" xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 24 24">
                                <path
                                    d="M21.92,11.6C19.9,6.91,16.1,4,12,4S4.1,6.91,2.08,11.6a1,1,0,0,0,0,.8C4.1,17.09,7.9,20,12,20s7.9-2.91,9.92-7.6A1,1,0,0,0,21.92,11.6ZM12,18c-3.17,0-6.17-2.29-7.9-6C5.83,8.29,8.83,6,12,6s6.17,2.29,7.9,6C18.17,15.71,15.17,18,12,18ZM12,8a4,4,0,1,0,4,4A4,4,0,0,0,12,8Zm0,6a2,2,0,1,1,2-2A2,2,0,0,1,12,14Z" />
