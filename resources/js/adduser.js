...
$(document).ready(function () {
    ...
    html += "</label>";
    html +=
        '<input type="text" placeholder="99999999" pattern="[0-9]+" minlength="8" maxlength="8" name="sapid[]" id="sapid--index--" required class="input input-bordered w-full bg-white border-slate-400 text-lg font-medium" data-user-index="--index--" />';
    html +=
        '<input id="username--index--" type="text" placeholder="ชื่อผู้ใช้งาน" class="mt-3 input w-full max-w-lg text-base disabled:border-none disabled:bg-white disabled:text-slate-900" disabled />';
    html += "</div>";
    ...
    html += "</label>";
    html +=
        '<select disabled required name="permission[]" id="permission--index--" class="select select-bordered disabled:bg-slate-200 bg-white border-slate-400 text-lg font-normal w-full">';
    ...
    html +=
        '<button id="remove" type="button" class="btn bg-red-500 border-none hover:bg-red-700">ลบช่องกรอกข้อมูล</button>';
    html += "</div>";
    html += "</div>";
    html += "</div>";
    html += "</section>";
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result.Status == "Active") {
                    if (result.Exist == "Yes") {
                        $("#username" + index).removeClass(
                            "disabled:text-teal-500"
                        );
                        $("#username" + index).addClass(
