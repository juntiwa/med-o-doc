...
class SendController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $log_activity->action = Auth::user()->username.' เข้าสู่หน้า "ทะเบียนหนังสือส่ง"';
    ...
    public function searchSend(Request $request)
    {
        ...
        if ($sfrommonth != '' && $stomonth != '') {
            $searchsends = $searchsends->whereBetween(DB::raw('MONTH(senddate)'), [$sfrommonth, $stomonth]);
        }

        if ($sfromyear != '' && $stoyear != '') {
            $searchsends = $searchsends->whereBetween(DB::raw('Year(senddate)'), [$sfromyear, $stoyear]);
        }
      ...
      ' เมื่อ  : เดือน '.$fmonth.'ปี '.$fyear.' ถึง  : เดือน '.$tmonth.'ปี '.$tyear;
        ...
        $log_activity->method = $request->method();
        $log_activity->user_agent = $request->header('user-agent');
        $log_activity->action = Auth::user()->username.' ค้นหาเอกสาร "ทะเบียนหนังสือส่ง"'.$searchLog;
