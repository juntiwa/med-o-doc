...
class RecController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $log_activity->action = Auth::user()->username.' เข้าสู่หน้า "ทะเบียนหนังสือรับ"';
    ...
    public function searchRec(Request $request)
    {
        ...
        if ($rfrommonth != '' && $rtomonth != '') {
            $searchrecs = $searchrecs->whereBetween(DB::raw('MONTH(letterrecs.recdate)'), [$rfrommonth, $rtomonth]);
        }
        if ($rfromyear != '' && $rtoyear != '') {
            $searchrecs = $searchrecs->whereBetween(DB::raw('Year(letterrecs.recdate)'), [$rfromyear, $rtoyear]);
        }
      ...
      ' เมื่อ  : เดือน '.$fmonth.'ปี '.$fyear.' ถึง  : เดือน '.$tmonth.'ปี '.$tyear;
        ...
        $log_activity->method = $request->method();
        $log_activity->user_agent = $request->header('user-agent');
        $log_activity->action = Auth::user()->username.' ค้นหาเอกสาร "ทะเบียนหนังสือรับ"'.$searchLog;
